...
Shader shBVHPrimitivesCS, shBVHConstructionCS;
Camera *cPlayer;

// BVH
uint gPrimitiveCapacity;

ConstantBuffer cbBVHGenerator;

RawStructuredBuffer<uint> sbPrimitiveCounter; // No UAV

StructuredBuffer<BVHNode> sbBVHNode;                // gPrimitiveCapacity * 2
StructuredBuffer<uint> sbBVHParent;                 // gPrimitiveCapacity * 2
StructuredBuffer<uint> sbBVHFlag;                   // (((gPrimitiveCapacity - 1) + 31) / 32); // bitfield for internal nodes
StructuredBuffer<uint> sbPrimitiveIDBuffer;         // gPrimitiveCapacity
StructuredBuffer<BVHPrimitive> sbPrimitive;         // gPrimitiveCapacity
StructuredBuffer<BVHPrimitiveData> sbPrimitiveData; // gPrimitiveCapacity
StructuredBuffer<float> sbPrimitiveMorton;          // gPrimitiveCapacity
...
void _DirectX::Load() {
    ...
    // 
    sbPrimitiveCounter. CreateDefault(1                                   , nullptr, false);
    sbBVHNode.          CreateDefault(gPrimitiveCapacity * 2              , nullptr, true );
    sbBVHParent.        CreateDefault(gPrimitiveCapacity * 2              , nullptr, true );
    sbBVHFlag.          CreateDefault(((gPrimitiveCapacity - 1) + 31) / 32, nullptr, true );
    sbPrimitiveIDBuffer.CreateDefault(gPrimitiveCapacity                  , nullptr, true );
    sbPrimitive.        CreateDefault(gPrimitiveCapacity                  , nullptr, true );
    sbPrimitiveData.    CreateDefault(gPrimitiveCapacity                  , nullptr, true );
    sbPrimitiveMorton.  CreateDefault(gPrimitiveCapacity                  , nullptr, true );

    // 
    BVHGenerator* cb = (BVHGenerator*)cbBVHGenerator.Map();
    cb->xBVHWorld               = DirectX::XMMatrixIdentity();
    cb->xBVHInstanceColor       = { .5f, .5f, .5f };
    cb->xBVHMaterialOffset      = 0;
    cb->xBVHMeshTriangleOffset  = 0;
    cb->xBVHMeshTriangleCount   = (uint)mModel.GetIB().GetNumber() / 3;
    cb->xBVHMeshVertexPOSStride = sizeof(Vertex_PNT);
    cbBVHGenerator.Unmap();
...
void _DirectX::Unload() {
    ...
    cbBVHGenerator.Release();

    sbPrimitiveCounter. Release();
    sbBVHNode.          Release();
    sbBVHParent.        Release();
    sbBVHFlag.          Release();
    sbPrimitiveIDBuffer.Release();
    sbPrimitive.        Release();
    sbPrimitiveData.    Release();
    sbPrimitiveMorton.  Release();
...
int main() {
    ...
    winCFG.Width  = 1024;
    winCFG.Height = 540;
    winCFG.Title = L"AVC Example - Luna Engine";

...
implTexture* Texture2::CreateTexture(std::variant<DXGI_FORMAT, UINT> format, D3D11_SUBRESOURCE_DATA *SubResource, 
        ...
        if( HasUAV && IsDepth ) {
            ...
            printf_s("[Texture::CreateTexture]: Warning: can't create DSV for texture with UAV! Creating %s type texture with UAV. [%s]\n", 
                     name[(formatTex == DXGI_FORMAT_R32_TYPELESS) * 2 + (formatTex == DXGI_FORMAT_R24G8_TYPELESS)], mName);
        }
        ...
        UINT Misc         = (HasMipMaps ? (MSAA ? 0 : D3D11_RESOURCE_MISC_GENERATE_MIPS) : 0) | 
        ...
        auto CreateTextureLocal = [formatTex, ArraySize, &res]
                (uint32_t dim, uint32_t mWidth, uint32_t mHeight, uint32_t mDepth,
        ...
        CreateTextureLocal(dim, mWidth, mHeight, mDepth, MipMaps, BindFlags, CPUAccess, Usage, 
                           SampleCount, Quality, Misc, subres, pTexture);

        if( FAILED(res) ) {
            printf_s("[Texture::Create]: Failed to create %uD texture! [%s]", dim, mName);
        } else if( MipMaps > 1 ) {
                        // Load mip data
            D3D11_BOX src;
            UINT mips = MipMaps;

            for( UINT i = 0, j = ArraySize / (1 + 5 * IsCube); i < j; i++ ) {
                local_texture pTexLocal;

                CreateTextureLocal(dim, mWidth, mHeight, mDepth, MipMaps, BindFlags, CPUAccess, Usage,
                                   SampleCount, Quality, Misc, &subres[i * mips], pTexLocal);
        ...
        if( HasUAV ) {
            if( Usage == D3D11_USAGE_STAGING ) {
                printf_s("[Texture::Create]: Can't create UAV for D3D11_USAGE_STAGING texture! [%s]", mName);
            } else {
                if( dim == 3 && ArraySize > 1 ) {
                    printf_s("[Texture::Create]: Can't create 3D UAV with ArraySize > 1! [%s]", mName);
                } else {
                    ...
                    res = gDirectX->gDevice->CreateUnorderedAccessView(Choose(pTexture._1D, pTexture._2D, pTexture._3D),
                                                                       &pUAVDesc, &pUAV);
                    if( FAILED(res) ) {
                        printf_s("[Texture::Create]: Failed to create UAV! [%s]", mName);
                    }
                }
            }
        }
        ...
        if( Usage != D3D11_USAGE_STAGING ) {
            ...
            pSRVDesc.ViewDimension = D3D11_SRV_DIMENSION(
                ...
                (dim == 2) ? (IsCube ? (D3D11_SRV_DIMENSION_TEXTURECUBE + (ArraySize > 1)) : (D3D11_SRV_DIMENSION_TEXTURE2D + (ArraySize > 1) + 2 * (Quality > 0))) :
                (dim == 3) ? D3D11_SRV_DIMENSION_TEXTURE3D : 
                D3D11_SRV_DIMENSION_UNKNOWN
            );

            if( dim == 1 ) {
                pSRVDesc.Texture1D.MipLevels       = MipMaps;
                ...
                pSRVDesc.Texture2D.MipLevels       = MipMaps;
                ...
                if( IsCube ) {
                    if( ArraySize == 6 ) {
                        pSRVDesc.TextureCube.MipLevels       = MipMaps;
                        pSRVDesc.TextureCube.MostDetailedMip = 0;
                    } else {
                        pSRVDesc.TextureCubeArray.MipLevels        = MipMaps;
                ...
                pSRVDesc.Texture3D.MipLevels       = MipMaps;
            ...
            res = gDirectX->gDevice->CreateShaderResourceView(Choose(pTexture._1D, pTexture._2D, pTexture._3D), 
                                                              &pSRVDesc, &pSRV);
            if( FAILED(res) ) {
                printf_s("[Texture::Create]: Failed to create SRV! [%s]", mName);
            }
        }
...
void Texture2::Load(std::string_view fname, UINT flags, uint32_t ArraySize) {
    ...
    if( ext == "dds" ) {
        ...
        if( res != Result::Success ) {
            ...
            printf_s("[Texture::Create]: Failed to load texture! [%s, %s]", fname.data(), mName);
            return;
        }
    ...
    } else { // Load png/jpeg/tga/hdr/etc
...
void Texture2::Bind(Shader::ShaderType type, UINT slot) {
    ...
    if( CPURead && CPUWrite ) {
        printf_s("[Texture::Bind]: Can't bind staging texture! [%s]", mName);
        return;
    }

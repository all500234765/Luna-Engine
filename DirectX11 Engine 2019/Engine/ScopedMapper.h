...
public:
    ...
    template<typename T2>
    ScopedMapCopy—ount(S* o, T2  * src, uint32_t count): obj(o) {
        data = (T*)obj->Map();
        if( data ) memcpy(&data[0], (void*)&src[0], sizeof(T) * count);
    }

    ScopedMapCopy—ount(S* o, void* src, uint32_t count): obj(o) {
        data = (T*)obj->Map();
        if( data ) memcpy(&data[0], &((T*)src)[0], sizeof(T) * count);
    }

    ScopedMapCopy—ount(S* o, T   * src, uint32_t count): obj(o) {
        data = (T*)obj->Map();
        if( data ) memcpy(&data[0], &src[0], sizeof(T) * count);
    }

    ~ScopedMapCopy—ount() { if( data ) obj->Unmap(); };
};

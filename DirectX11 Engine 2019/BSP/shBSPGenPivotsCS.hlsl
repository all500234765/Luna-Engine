...
Pivot EvaluatePivot(int pivot) {
    ...
    for( uint i = 0; i < _IndexCount; i += 3 ) {
        uint split = SplitType(Plane2VertexSign(plane, _Position[i + 0]), 
                               Plane2VertexSign(plane, _Position[i + 1]), 
                               Plane2VertexSign(plane, _Position[i + 2]));
        ...
        switch( split ) {
            ...
            case 4: //  0 + 4 + 0 = 4;
            case 5: //  0 + 4 + 1 = 5;
            case 16: // 16 + 0 + 0 = 16; (split & 0x11) > 0
            case 17: // 16 + 0 + 1 = 17; 
            case 20: // 16 + 4 + 0 = 20; frac(split / 20.f) == 0.f
              e.back++;
              break;

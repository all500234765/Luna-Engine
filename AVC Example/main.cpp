...
bool _DirectX::FrameFunction() {
    ...
    gContext->Draw(vbAVC.GetNumber(), 0);
...
void _DirectX::Load() {
    // Create blocks
    UINT Num = 8 * 8 * 8;
    std::vector<AVC_Cube> vertices;
    vertices.reserve(Num);
    for( UINT i = 0; i < Num; i++ ) {
        AVC_Cube a = {
            { (float)(i % 8), 1.f*(float)(i / 64), (float)((i % 64) / 8) },
            16.f
        };
    ...
    vbAVC.CreateDefault(Num, sizeof(AVC_Cube), &vertices[0]);

    // Load AVC shaders
    shRenderAVC.LoadFile("shRenderAVCCubeVS.cso", Shader::Vertex);
    shRenderAVC.LoadFile("shRenderAVCCubeGS.cso", Shader::Geometry);
    shRenderAVC.LoadFile("shRenderAVCCubePS.cso", Shader::Pixel);
    shRenderAVC.ReleaseBlobs();
